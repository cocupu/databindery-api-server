...
describe Bindery::PoolImporter do
  ...
  describe "import_from" do
    it "imports pool, models and nodes from json" do
      ...
      expect(imported_pool.models.count).to eq(2) # Should re-use existing canonical FileEntity model instead of importing it
      ...
      expect(imported_pool.nodes.where(model:Model.file_entity).count ).to eq(5)
  ...
  describe "import_models" do
    it "imports all the models, rebuilding all of their fields, sets label_field, and retains association :references values" do
      ...
      expect(pool.models.count).to eq(original_model_count+2)
