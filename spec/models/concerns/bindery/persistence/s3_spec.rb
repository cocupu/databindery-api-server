...
describe Bindery::Persistence::S3 do
  it "should have default connection" do
    conn = subject.default_connection 
    expect(conn).to be_instance_of S3Connection
  ...
  describe "key_from_filepath" do
    it "should extract the object key from the filepath from response that direct upload returns" do
      ...
      key = Bindery::Persistence::S3.key_from_filepath(filepath,bucket:"f53e6340-66e4-0130-8d3f-442c031da886")
  ...
  describe "generate_storage_location_id" do
    ...
    it "should generate the desired S3 key for the given File Entity" do
      ...
      expect(Bindery::Persistence::S3.generate_storage_location_id(@file_entity)).to eq "samplePid_#{@formatted_now}"
    end
    it "should include filename if available" do
      Time.stub(:now).and_return(@now)
      @file_entity.file_name ="my file.jpg"
      expect(Bindery::Persistence::S3.generate_storage_location_id(@file_entity)).to eq "samplePid_#{@formatted_now}_my file.jpg"
    end
  end
end
